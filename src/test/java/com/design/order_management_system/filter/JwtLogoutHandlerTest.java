...
class JwtLogoutHandlerTest {
    ...
    void logout_WhenTokenMissing_ShouldResolveMissingTokenException() {
        ...
        when(exceptionResolver.resolveException(
                        eq(request), eq(response), eq(null), any(MissingTokenException.class)))
                .thenReturn(result);
        ...
        verify(exceptionResolver)
                .resolveException(eq(request), eq(response), eq(null), exceptionCaptor.capture());
        ...
        Assertions.assertThat(exception.getMessage())
                .isEqualTo(ErrorMessageConstants.BEARER_TOKEN_MISSING);
    ...
    void logout_WhenTokenDoesNotHaveBearerPrefix_ShouldThrowMissingTokenException() {
        ...
        when(request.getHeader(CommonConstants.AUTHORIZATION_HEADER_KEY))
                .thenReturn(GeneratorUtils.generateUUID());
        when(exceptionResolver.resolveException(
                        eq(request), eq(response), eq(null), any(MissingTokenException.class)))
                .thenReturn(result);
