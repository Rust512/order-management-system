...
public class OrderItemControllerIT extends DatabaseTest {
  ...
  @DisplayName(
      """
            ...
            """)
  void
      addOrderItem_WhenDraftOrderWithAnOrderItemWithGivenProductIdExists_ShouldUpdateTheExistingOrderItemAndReturnStatus200() {
    String productName = "Prd01";
    BigDecimal productPrice = BigDecimal.TEN;
    long stock = 6L;
    long reservedStock = 0L;
    ...
    var updatedRequestBody =
        OrderItemRequest.builder().productId(productId).quantity(additionalQuantity).build();

    var updatedRequest = new HttpEntity<>(updatedRequestBody, headers);

    ResponseEntity<OrderResponse> updatedResponse =
        restTemplate.postForEntity("/v1/orders/items", updatedRequest, OrderResponse.class);
  ...
  void deleteOrderItem_WithValidProductId_ShouldDeleteOrderItemAndReturnStatus200() {
    ...
    URI uri =
        UriComponentsBuilder.fromUriString("/v1/orders/items/{productId}")
            .buildAndExpand(productId)
            .toUri();
    ResponseEntity<OrderResponse> updatedResponse =
        restTemplate.exchange(uri, HttpMethod.DELETE, updateRequest, OrderResponse.class);
    ...
    Assertions.assertThat(updatedResponseBody.getTotalPrice())
  ...
  private void setAuthorizationHeader(HttpHeaders headers) {
    ...
    loginRequest.setUsername(OrderItemControllerIT.NORMAL_USERNAME);
    loginRequest.setPassword(OrderItemControllerIT.NORMAL_PASSWORD);
