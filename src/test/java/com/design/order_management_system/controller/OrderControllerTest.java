...
class OrderControllerTest {
    ...
    void getOrder_WhenUserAdminAndOrderMissing_ShouldReturnStatusCode404() throws Exception {
        ...
        var expectedMessage = String.format(ErrorMessageConstants.RESOURCE_NOT_FOUND, CommonConstants.ORDER,
                "id",
                orderId);

        when(orderService.getOrderById(orderId)).thenThrow(new ResourceNotFoundException(CommonConstants.ORDER, "id", String.valueOf(orderId)));
        ...
        mockMvc.perform(get("/v1/orders/{id}", orderId))
                ...
                .andExpect(jsonPath("$.sExceptionName").value(ResourceNotFoundException.class.getSimpleName()));
    ...
    void getOrder_WhenUserNotAdminAndOrderMissing_ShouldReturnStatusCode403() throws Exception {
        ...
        var expectedMessage = String.format(ErrorMessageConstants.RESOURCE_NOT_OWNED, CommonConstants.ORDER,
                "id",
                orderId,
                userId);

        when(orderService.getOrderById(orderId)).thenThrow(new ResourceNotOwnedException(CommonConstants.ORDER, "id", String.valueOf(orderId), userId));
        ...
        mockMvc.perform(get("/v1/orders/{id}", orderId))
                ...
                .andExpect(jsonPath("$.sMessage").value(expectedMessage))
                .andExpect(jsonPath("$.sPath").value(String.format("/v1/orders/%d", orderId)))
                .andExpect(jsonPath("$.sExceptionName").value(ResourceNotOwnedException.class.getSimpleName()));
