...
class ProductControllerIntegrationTest extends DatabaseTest {
    ...
    void updateProduct_WhenProductExists_ShouldUpdateProductAndReturnStatus200() {
        ...
        var uri = UriComponentsBuilder.fromUriString("/v1/products/{productId}").buildAndExpand(productId).toUri();
        ...
        transactionTemplate.executeWithoutResult(_ -> {
            var persistedProduct = productRepository.findById(updateBody.getProductId()).orElseThrow();
            Assertions.assertThat(persistedProduct).isNotNull();
    ...
    void getProducts_ShouldReturnAllExistingProducts() {
        ...
        var stock0 = 2L;
        var stock1 = 3L;
        var productId0 = transactionTemplate.execute(_ -> {
        ...
        var productId1 = transactionTemplate.execute(_ -> {
        ...
        var page = 0;
        var size = 1;
        var uri = UriComponentsBuilder.fromUriString("/v1/products")
                .queryParam("page", page)
                .queryParam("size", size)
                .encode()
                .toUriString();

        var response = this.restTemplate.exchange(uri, HttpMethod.GET, requestEntity, PagedResponse.class);
