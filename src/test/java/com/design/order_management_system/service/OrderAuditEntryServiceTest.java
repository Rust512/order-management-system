...
class OrderAuditEntryServiceTest {
    ...
    @Mock private OrderAuditEntryRepository orderAuditEntryRepository;
    @Mock private OrderItemSnapshotService orderItemSnapshotService;
    @Mock private OrderAuditEntryToResponse orderAuditEntryToResponse;

    @InjectMocks private OrderAuditEntryService orderAuditEntryService;
    ...
    void saveOrderAuditEntry_WhenUserDoesNotExist_ShouldThrowResourceNotFoundException() {
        ...
        TestSecurityUtils.setAuthenticationContext(
                userId, GeneratorUtils.generateUUID(), CommonConstants.ROLE_ADMIN);
        ...
        Assertions.assertThatThrownBy(
                        () -> orderAuditEntryService.saveOrderAuditEntry(userId, order, operation))
        ...
        verifyNoInteractions(
                orderAuditEntryRepository, orderItemSnapshotService, orderAuditEntryToResponse);
    }
    ...
    @DisplayName(
            ...
            """)
    void
            getOrderAuditEntryByVersion_WhenAdminUserAndOrderDoesNotExist_ShouldThrowResourceNotFoundException() {
        TestSecurityUtils.setAuthenticationContext(
                1L, GeneratorUtils.generateUUID(), CommonConstants.ROLE_ADMIN);

        var orderId = 2L;
        var version = 1L;

        when(orderAuditEntryRepository.findByOrderIdAndVersion(orderId, version))
        ...
        Assertions.assertThatThrownBy(
                        () -> orderAuditEntryService.getOrderAuditEntryByVersion(orderId, version))
                .isInstanceOf(ResourceNotFoundException.class)
                .hasMessage(expectedMessage);
    ...
    @DisplayName(
            ...
            """)
    void
            getOrderAuditEntryByVersion_WhenUserNotAdminAndOrderDoesNotExist_ShouldThrowResourceNotFoundException() {
        TestSecurityUtils.setAuthenticationContext(
                1L, GeneratorUtils.generateUUID(), CommonConstants.ROLE_USER);

        var orderId = 2L;
        var version = 1L;

        when(orderAuditEntryRepository.existsByOrderIdAndVersion(orderId, version))
                .thenReturn(false);
    ...
    @DisplayName(
            ...
            """)
    void
            getOrderAuditEntryByVersion_WhenUserNotAdminAndOrderExistsButNotOwned_ShouldThrowResourceNotOwnedException() {
        var userId = 1L;
        TestSecurityUtils.setAuthenticationContext(
                userId, GeneratorUtils.generateUUID(), CommonConstants.ROLE_USER);
        ...
        when(orderAuditEntryRepository.existsByOrderIdAndVersion(orderId, version))
                .thenReturn(true);
        when(orderAuditEntryRepository.findByOrderIdAndUserIdAndVersion(orderId, userId, version))
                .thenReturn(Optional.empty());
        ...
        verify(orderAuditEntryRepository, never()).findByOrderIdAndVersion(orderId, version);
        verify(orderAuditEntryRepository).existsByOrderIdAndVersion(orderId, version);
        verify(orderAuditEntryRepository)
