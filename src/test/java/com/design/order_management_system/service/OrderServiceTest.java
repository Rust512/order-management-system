...
class OrderServiceTest {
    ...
    @Mock private OrderItemToOrderItemResponse orderItemToOrderItemResponse;
    ...
    void getOrderById_WhenUserIsAdminAndOrderFound_ShouldReturnTheCorrespondingOrder() {
        ...
        TestSecurityUtils.setAuthenticationContext(
                USER_ID, GeneratorUtils.generateUUID(), CommonConstants.ROLE_ADMIN);
    ...
    void getOrderById_WhenUserNotAdminAndOrderFound_ShouldReturnTheCorrespondingOrder() {
        ...
        when(orderRepository.getOrderByIdAndUserIdWithItems(orderId, USER_ID))
    ...
    @DisplayName(
            ...
            """)
    void
            getOrderById_WhenUserNotAdminAndOrderFoundButNowOwned_ShouldThrowResourceNotFoundException() {
        var orderId = 1L;
        TestSecurityUtils.setAuthenticationContext(
                USER_ID, GeneratorUtils.generateUUID(), CommonConstants.ROLE_USER);
    ...
    void getDraftOrder_WhenConcurrentSaveOccurs_ShouldReturnExistingOrder() {
        ...
        when(orderRepository.save(any(Order.class)))
                .thenThrow(new DataIntegrityViolationException("concurrent creation"));
        ...
        verify(orderRepository, times(2))
                .fetchDraftOrderWithOrderItems(USER_ID, OrderStatus.CREATED);
    ...
    void checkoutOrder_WhenDraftOrderIsEmpty_ShouldThrowResourceNotFoundException() {
        ...
        var orderId = 1L;
        var order = Order.builder().id(orderId).orderStatus(OrderStatus.CREATED).build();

        when(orderRepository.fetchDraftOrderWithOrderItems(USER_ID, OrderStatus.CREATED))
                .thenReturn(Optional.of(order));
        when(orderItemRepository.findAllByOrder_IdForRead(orderId))
