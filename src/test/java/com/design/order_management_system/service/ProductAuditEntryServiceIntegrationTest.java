...
class ProductAuditEntryServiceIntegrationTest extends DatabaseTest {
    ...
    void getProductVersions_ShouldReturnAllAuditEntriesForTheGivenProductId() {
        ...
        var uri = UriComponentsBuilder.fromUriString("/v1/products/{productId}")
                .buildAndExpand(productId)
                .toUri();

        var productUpdateResponse = restTemplate.exchange(uri, HttpMethod.PUT, updateRequestEntity, ProductResponse.class);
        ...
        var page = 0;
        var size = 1;
        var auditLogsUri = UriComponentsBuilder.fromUriString("/v1/products/{productId}/audit")
        ...
        var auditLogResponse = restTemplate.exchange(auditLogsUri, HttpMethod.GET, auditLogRequestEntity, PagedProductAuditEntryResponse.class);
    ...
    @DisplayName(value = """
            ...
            """)
    void getProductAuditEntryByVersion_WhenAuditEntryWithGivenVersionExists_ShouldReturnTheSameAuditEntry() {
        var productName = GeneratorUtils.generateUUID();
        var price = BigDecimal.TWO;
        var stock = 2L;
        var request = CreateProductRequest.builder()
        ...
        var size = 1;
        var version = 2;
        var auditLogsUri = UriComponentsBuilder.fromUriString("/v1/products/{productId}/audit/{version}")
                .queryParam("page", page)
                .queryParam("size", size)
        ...
        var auditLogRequestEntity = new HttpEntity<>(headers);
        var auditLogResponse = restTemplate.exchange(auditLogsUri, HttpMethod.GET, auditLogRequestEntity, ProductAuditEntryResponse.class);
