...
class LoginServiceTest {
    ...
    void getToken_WhenUsernameExistsAndPasswordMismatch_ShouldThrowException() {
        ...
        var user = User.builder().username(username).password(hashedPassword).build();
    ...
    void getToken_WhenUsernameExistsAndPasswordCorrect_ShouldReturnCorrectJwtToken() {
        ...
        var user = User.builder().id(1L).username(username).password(hashedPassword).build();
        ...
        List<String> roles = rawList.stream().map(Objects::toString).toList();
