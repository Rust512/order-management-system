...
public class SecurityUtils {
    ...
    private static final String INVALID_AUTHENTICATION_PRINCIPAL =
    ...
    public static String generateJwtToken(PrincipalUser user) {
        ...
        var claims = Map.of(ROLES, roles, USER_ID, user.getUserId());
        return Jwts.builder()
                .claims(claims)
                .subject(user.getUsername())
                .expiration(
                        Date.from(Instant.now().plus(TOKEN_EXPIRY_IN_MINUTES, ChronoUnit.MINUTES)))
    ...
    public static PrincipalUser getPrincipalUserFromAuthentication(Authentication authentication) {
        ...
        if (!(principal instanceof PrincipalUser principalUser)) {
            ...
            log.error(
                    "Security context invalid; principalType={} reason=invalid_principal",
                    principalType);
