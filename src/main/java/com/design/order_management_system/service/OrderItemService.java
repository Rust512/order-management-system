...
public class OrderItemService {
    ...
    public OrderResponse addOrderItem(OrderItemRequest orderItemRequest) {
        ...
        if (lockedOrderItem.isPresent()) {
            ...
            log.info(
                    "Creating new order item; userId={} productId={} orderId={}",
                    userId,
                    productId,
                    orderId);
            orderItem =
        ...
        var draftOrder =
                orderRepository
                        ...
                        .orElseThrow(
                                () -> {
                                    log.warn(
                                            "Edit order item failed; userId={} reason=order_not_found",
                                            userId);
                                    return new ResourceNotFoundException(
                ...
                draftOrder.getOrderItems().stream()
                        ...
                        .orElseThrow(
                                () -> {
                                    ...
                                    return new IllegalStateException(
                                            String.format(
                                                    "Order item with ID %s unexpectedly missing",
                                                    orderItemId));
                                });
        ...
        var draftOrder =
                orderRepository
                        ...
                        .orElseThrow(
                                () -> {
                                    log.warn(
                                            "Delete order item failed; userId={} reason=order_not_found",
                                            userId);
                                    return new ResourceNotFoundException(
                                            CommonConstants.ORDER,
                                            "userId",
