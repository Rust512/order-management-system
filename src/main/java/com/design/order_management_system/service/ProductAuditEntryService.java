...
public class ProductAuditEntryService {
    ...
    void createProductAuditEntry(Long userId, Product product, OperationType operationType) {
        ...
        var user =
                userRepository
                        ...
                        .orElseThrow(
                                () -> {
                                    log.warn(
                                            "product audit entry creation failed; userId={} productId={} reason=user_not_found",
                                            userId,
                                            productId);
        ...
        var nextVersion =
                productAuditEntryRepository.getNextAuditVersionByProductId(product.getId());

        var productAuditEntry =
                ProductAuditEntry.builder()
                        .version(nextVersion)
        ...
        log.info(
                "Product audit entry created; userId={} productId={} auditEntryId={}",
                userId,
                productId,
    ...
    public PagedResponse<ProductAuditEntryResponse> getProductVersions(
            Long productId, Pageable pageable) {
        var userId = SecurityUtils.getPrincipalUser().getUserId();
        if (!productRepository.existsById(productId)) {
            log.warn(
                    "Product audit entry fetch failed; userId={} productId={} reason=product_not_found",
                    userId,
                    productId);
        ...
        var auditEntries = result.getContent().stream().map(productAuditEntryToResponse).toList();
    ...
    public ProductAuditEntryResponse getProductAuditEntryByVersion(Long productId, Long version) {
        var userId = SecurityUtils.getPrincipalUser().getUserId();
        return productAuditEntryRepository
                .findByProduct_IdAndVersion(productId, version)
                .map(productAuditEntryToResponse)
                .orElseThrow(
                        () -> {
                            log.warn(
                                    "Product audit entry fetch failed; userId={} productId={} version={} reason=audit_entry_not_found",
                                    userId,
                                    productId,
                                    version);
                            return new ResourceNotFoundException(
                                    ...
                                    "(productId, version)",
                                    String.format("(%d, %d)", productId, version));
                        });
    }
}
