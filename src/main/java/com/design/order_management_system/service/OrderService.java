...
public class OrderService {
    ...
    public OrderResponse getOrderById(Long id) {
        ...
        if (SecurityUtils.isAdmin(principalUser)) {
            ...
            return orderToOrderResponse.apply(
                    orderRepository
                            .getOrderByIdWithItems(id)
                            .orElseThrow(
                                    () -> {
                                        log.warn(
                                                "Fetch order failed; userId={} orderId={} reason=order_not_found",
                                                userId,
                                                id);
                                        return new ResourceNotFoundException(
                                                CommonConstants.ORDER, "id", String.valueOf(id));
                                    }));
        }

        if (!orderRepository.existsById(id)) {
            log.warn("Fetch order failed; userId={} orderId={} reason=order_not_found", userId, id);
            throw new ResourceNotFoundException(CommonConstants.ORDER, "id", String.valueOf(id));
        }
        ...
        var fetchedOrder =
                orderRepository
                        .getOrderByIdAndUserIdWithItems(id, userId)
                        .orElseThrow(
                                () -> {
                                    log.warn(
                                            "Fetch order failed; userId={} orderId={} reason=order_not_accessible",
                                            userId,
                                            id);
    ...
    public Order getDraftOrder(Long userId) {
        var optionalOrder =
                orderRepository.fetchDraftOrderWithOrderItems(userId, OrderStatus.CREATED);
        ...
        var user =
                userRepository
                        ...
                        .orElseThrow(
                                () -> {
                                    log.info(
                                            "Get draft order failed; userId={} reason=user_not_found",
                                            userId);
                                    return new ResourceNotFoundException(
                                            CommonConstants.USER, "id", String.valueOf(userId));
                                });
        var order = Order.builder().orderStatus(OrderStatus.CREATED).user(user).build();

        try {
            var savedOrder = orderRepository.save(order);
            var orderId = savedOrder.getId();

            orderAuditEntryService.saveOrderAuditEntry(
                    userId, savedOrder, OrderOperation.CREATE_ORDER);

            log.info("Draft order retrieved; userId={} orderId={}", userId, orderId);
            return savedOrder;
        } catch (DataIntegrityViolationException ex) {
            log.info(
                    "Draft order creation retried; userId={} reason=concurrent_creation",
                    userId,
                    ex);
                    ...
                    .orElseThrow(
                            () ->
                                    new ResourceNotFoundException(
                                            CommonConstants.ORDER, "user", String.valueOf(userId)));
        }
    }
    ...
    public OrderResponse checkoutOrder() {
        ...
        var draftOrder =
                orderRepository
                        ...
                        .orElseThrow(
                                () -> {
                                    log.warn(
                                            "Order checkout failed; userId={} reason=order_not_found",
                                            userId);
                                    return new ResourceNotFoundException(
                                            CommonConstants.ORDER,
                                            "user_id",
        ...
        if (orderItems.isEmpty()) {
            ...
            throw new ResourceNotFoundException(
                    CommonConstants.ORDER_ITEM, "order_id", String.valueOf(orderId));
        }

        var productIdToOrderItemMap =
                orderItems.stream()
                        .collect(
                                Collectors.toMap(
                                        item -> item.getProduct().getId(), Function.identity()));

        var productIds = productIdToOrderItemMap.keySet().stream().sorted().toList();

        productRepository
                .findAllByIdInForWrite(productIds)
                .forEach(
                        product -> {
        ...
        var draftOrder =
                orderRepository
                        ...
                        .orElseThrow(
                                () -> {
                                    log.warn(
                                            "Order cancel failed; userId={} reason=order_not_found",
                                            userId);
                                    return new ResourceNotFoundException(
                                            CommonConstants.ORDER,
                                            "user_id",
