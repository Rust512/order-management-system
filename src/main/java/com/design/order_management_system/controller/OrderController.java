...
public class OrderController {
  ...
  ResponseEntity<OrderResponse> getOrder(
      @PathVariable @Parameter(description = "The order ID", example = "2") Long id) {
        ...
        @ApiResponse(
            ...
            content =
                @Content(
                    ...
                    examples =
                        @ExampleObject(
                            value = ErrorResponseExamples.ORDER_AUDIT_ENTRIES_NOT_FOUND)))
      })
  ResponseEntity<PagedResponse<OrderAuditEntryResponse>> getAudit(
      @PathVariable @Positive @Parameter(description = "The order ID", example = "2") Long orderId,
      @ParameterObject @PageableDefault(size = 5, sort = "version", direction = Sort.Direction.ASC)
          Pageable pageable) {
        ...
        @ApiResponse(
            ...
            content =
                @Content(
                    schema = @Schema(implementation = ApiErrorResponse.class),
                    examples =
                        @ExampleObject(value = ErrorResponseExamples.ORDER_AUDIT_ENTRY_NOT_OWNED)))
      })
  ResponseEntity<OrderAuditEntryResponse> getAuditByVersion(
      @PathVariable @Positive @Parameter(description = "The order ID", example = "2") Long orderId,
      @PathVariable @Positive @Parameter(description = "The audit entry version", example = "3")
          Long version) {
